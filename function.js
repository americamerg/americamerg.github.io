let caption = {
    ...
    'engagement/wedding': [
        'if you like it then you shouldve put a ringt on it',
        'this year'],
    'birthday': [
        'its my birthday',
        'party party party',
...
function renderCategories() {
    // render the categories as options
    let ret = ''

    for  (const [key, value] of Object.entries(caption)){
...
function displayCaption() {
    ...
    //convert it into between 0 to length of captions[]

    let set = caption[category]
    let index = Math.floor(Math.random() * set.length)

    //display the caption of that index
    let div = document.querySelector('#caption')
    div.innerHTML = renderCaption(set[index])
